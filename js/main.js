      ...
      (async function () {
        try {
          if (window.PSSCloud) {
            ...
            await window.PSSCloud.addInquiry(inquiry);
          } else {
            const inquiries = JSON.parse(localStorage.getItem('pss-inquiries') || '[]');
            inquiries.push(inquiry);
            localStorage.setItem('pss-inquiries', JSON.stringify(inquiries));
          }
        } catch (err) {
          try {
            const inquiries = JSON.parse(localStorage.getItem('pss-inquiries') || '[]');
