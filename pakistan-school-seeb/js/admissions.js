...
(function () {
  ...
  function renderFiles() {
    ...
    fileList.innerHTML = uploadedFiles.map((name, i) =>
      `<div style="display:flex;align-items:center;gap:0.5rem;padding:0.4rem 0;font-size:0.9rem">
        <span style="color:var(--pk-emerald)">📄</span> ${name}
  ...
  if (submitBtn) {
    submitBtn.addEventListener('click', () => {
      ...
      const studentName = wizard.querySelector('[name="studentName"]')?.value || '';
      const grade = wizard.querySelector('[name="gradeApplied"]')?.value || '';
      const email = wizard.querySelector('[name="email"]')?.value || '';
  ...
  if (trackBtn && trackInput && statusResult) {
    trackBtn.addEventListener('click', () => {
      ...
      const app = apps.find(a => a.trackingId === id);
      if (!app) {
